...
export type DashboardBandHeight = "kpi" | "hero" | "standard" | "strip" | "list";

export type BandIdentifier =
  | "A"
  | "B"
  | "B2"
  | "C"
...
export interface DashboardBand {
  band: BandIdentifier;
  height?: DashboardBandHeight | number;
...
export interface TimeInStageData {
  ...
  slowestStage: string;
  departmentAvgDays?: number;
}

export interface BudgetBurnVsElapsedData {
  consumedPercent: number;
...
export interface RequestThroughputData {
  weeks: WeeklyThroughputItem[];
...
export interface WidgetDataMap {
  ...
  "budget-burn-vs-elapsed": BudgetBurnVsElapsedData;
  "request-throughput": RequestThroughputData;
  ...
  "milestone-timeline": UpcomingMilestonesData;
