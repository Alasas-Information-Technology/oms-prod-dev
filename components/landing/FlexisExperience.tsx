...
function FlexisCard({ model, onClick }: { model: any; onClick: () => void }) {
  return (
    <motion.div
      ...
      className="bg-card border border-border/70 rounded-xl p-6 sm:p-8 cursor-pointer overflow-hidden relative group hover:shadow-xl hover:shadow-primary/5 hover:border-primary/40 h-full flex flex-col"
    >
      ...
      <div className="flex items-start justify-between relative z-10 mb-6">
        <div className="size-14 sm:size-16 rounded-xl bg-secondary/60 flex items-center justify-center group-hover:bg-primary group-hover:text-white transition-colors duration-300">
          <span className="text-2xl sm:text-3xl font-extrabold text-primary group-hover:text-white transition-colors">
            {model.id}
          </span>
        </div>
        <div className="size-8 rounded-full border border-border/60 flex items-center justify-center group-hover:bg-primary group-hover:border-primary transition-all">
          <ArrowRight className="size-4 text-muted-foreground group-hover:text-white transition-colors" />
        </div>
      </div>
      
      <div className="relative z-10 flex-grow">
        <h3 className="text-xl sm:text-2xl font-bold text-heading mb-2 leading-snug tracking-tight">
          {model.title}
        </h3>
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 md:p-6 lg:p-12">
      ...
      <motion.div
        ...
        className="relative w-full h-full md:h-auto md:max-h-[90vh] md:max-w-4xl bg-card border-border md:rounded-2xl shadow-2xl flex flex-col overflow-hidden"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 sm:px-8 py-6 border-b border-border/50 bg-secondary/30">
          <div className="flex items-center gap-4">
            <div className="size-12 rounded-xl bg-primary flex items-center justify-center text-white shadow-xs">
              <span className="text-2xl font-bold">{model.id}</span>
            </div>
            <div>
              <span className="text-xs font-bold uppercase tracking-wider text-primary block mb-0.5">FLEXIS Model</span>
              <h2 className="text-xl sm:text-2xl md:text-3xl font-extrabold text-heading tracking-tight leading-tight">{model.title}</h2>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6 sm:p-8 md:p-10">
          <p className="text-base sm:text-lg md:text-xl text-heading font-medium leading-relaxed mb-8 sm:mb-10 max-w-2xl">
            {model.description}
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
            ...
            <div className="space-y-8">
              <div>
                <h4 className="text-xs uppercase tracking-wider text-muted-foreground font-bold mb-3 flex items-center gap-2">
                  <span className="w-3 h-0.5 bg-primary rounded-full"></span>
                  Ideal For
                </h4>
                <p className="text-sm sm:text-base text-foreground bg-secondary/40 p-4 rounded-xl border border-border/50 leading-relaxed font-medium">
                  {model.idealFor}
                </p>
              </div>
                ...
                <ul className="space-y-2.5">
                  {model.useCases.map((uc: string, i: number) => (
                    <li key={i} className="flex items-start gap-3 text-sm text-foreground">
                      ...
                      <span className="leading-relaxed font-normal">{uc}</span>
                    </li>
                  ))}
                </ul>
              </div>
            ...
            <div className="space-y-8">
              <div>
                ...
                <ul className="space-y-2.5">
                  {model.benefits.map((b: string, i: number) => (
                    <li key={i} className="flex items-start gap-3 text-sm text-foreground">
