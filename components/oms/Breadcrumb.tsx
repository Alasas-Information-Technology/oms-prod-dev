...
/**
 * Enterprise Breadcrumb Component per Part 5 of docs/APP-SHELL-SPEC.md
 * 
 * Rules:
 * 1. Type scale: Ancestors 14px/400/--text-secondary, Separator 14px/--text-muted, Current page 15px/500/--text-primary.
 * 2. Separator is "/" with 8px each side.
 * 3. No home icon.
 * 4. Final crumb is not a link, has no hover state, carries aria-current="page".
 * 5. Truncation: 1-4 crumbs show all; 5+ shows first, then "…" dropdown menu of hidden levels, then last two.
 * 6. Any single crumb > 28 characters truncates with an ellipsis and a tooltip carrying full name.
 */
        ...
        {visibleCrumbs.map((crumb: any, index) => {
          ...
          return (
            <React.Fragment key={crumb.href || crumb.label + index}>
              {/* Separator: "/" at 14px with 8px each side (mx-2) */}
              {index > 0 && (
                <li
                  aria-hidden="true"
              ...
              {isEllipsis ? (
                ...
                /* Current page crumb: 15px font-medium text-foreground, aria-current="page", not a link */
                ...
                /* Ancestor crumb: 14px font-normal text-muted-foreground */
