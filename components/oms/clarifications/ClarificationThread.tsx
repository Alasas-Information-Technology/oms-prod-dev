...
/**
 * Maps technical action codes to friendly plain-language descriptions per Task 4
 */
...
export function ClarificationThread({
  ...
  const renderThreadEntry = (entry: ClarificationThreadEntry, isLatest = false) => {
    ...
    const plainAction = getActionPlainLanguage(entry.action, isRequestor);
            ...
            <div className="flex flex-wrap gap-2">
              {entry.attachments.map((att) => (
                ...
                >
                  ...
                  <span className="text-[10px] text-muted-foreground">({formatBytes(att.sizeBytes)})</span>
                  {att.url && (
                    <a
                      href={att.url}
                      target="_blank"
  ...
  return (
    <div className={cn("space-y-3", className)}>
      ...
      <div className="flex items-center justify-between">
        ...
        {/* "No limit on clarification cycles" appears only when cycleNumber >= 2 per Part 1.5 */}
        {cycleNumber >= 2 && (
      ...
      {/* Earlier Messages Collapsible Section (when earlier entries exist) */}
      ...
      {latestEntry && renderThreadEntry(latestEntry, true)}
    </div>
  );
}
