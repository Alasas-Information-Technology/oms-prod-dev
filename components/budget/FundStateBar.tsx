...
export function FundStateBar({
  ...
  if (!reconciliation.isReconciled) {
    ...
    return (
      <div
        ...
        className={cn(
          "p-4 rounded-2xl border border-rose-300 bg-rose-50/90 dark:border-rose-900/60 dark:bg-rose-950/40 text-rose-900 dark:text-rose-200 shadow-xs space-y-2 animate-in fade-in-50",
          className
        )}
        {...props}
      >
  ...
  return (
    ...
    >
      ...
      {/* Stacked Bar */}
      <div className="relative h-6 w-full overflow-hidden rounded-xl bg-muted/60 flex shadow-inner">
        {segments.map((seg) => {
            ...
            >
              ...
              <div className="flex items-baseline justify-between gap-1">
                <span className="text-xs font-bold text-foreground font-mono tabular-nums">
                  {currency} {formatAmount(seg.fils)}
                </span>
                <span className={cn("text-[11px] font-semibold font-mono", seg.textColor)}>
                  {seg.percentString}
                </span>
              </div>
            </button>
