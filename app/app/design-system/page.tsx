  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="mb-6 pb-4 border-b border-slate-200">
        ...
        <p className="text-sm text-muted-foreground mt-0.5">OMS Design System · Component Library v1.0 · Government Economic Zone Authority</p>
      </div>
      {children}
    </div>
  );
...
function TypographySection() {
  const scale = [
    { name: "Display", cls: "text-3xl font-bold tracking-tight text-slate-900", sample: "Government Economic Zone Authority" },
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="flex gap-3">
          ...
          <Dialog>
            <DialogTrigger asChild>
              <Button variant="destructive"><Trash2 size={14} /> Delete Record</Button>
            </DialogTrigger>
            <DialogContent className="sm:max-w-[400px]">
              <DialogHeader>
                <DialogTitle className="flex items-center gap-2"><AlertCircle size={18} className="text-destructive" /> Confirm Deletion</DialogTitle>
                <DialogDescription>This action is permanent and cannot be undone. The contract record OMS-2025-001 and all associated documents will be removed from the system.</DialogDescription>
              </DialogHeader>
              <DialogFooter>
                <Button variant="ghost">Cancel</Button>
                <Button variant="destructive"><Trash2 size={14} /> Delete Permanently</Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
        </div>
...
const KPI_CARDS = [
  { label: "Total Contracts", value: "1,247", sub: "FY 2025 registry", trend: "+12%", up: true, Icon: FileText, color: "text-primary", bg: "bg-primary/10" },
  { label: "Active Vendors", value: "89", sub: "3 new this month", trend: "+3.5%", up: true, Icon: Users, color: "text-emerald-600", bg: "bg-emerald-50" },
  { label: "Procurement Value", value: "₱2.83B", sub: "68% budget utilised", trend: "+8.2%", up: true, Icon: DollarSign, color: "text-indigo-600", bg: "bg-indigo-50" },
  { label: "Pending Approvals", value: "23", sub: "5 marked urgent", trend: "-4", up: false, Icon: Clock, color: "text-amber-600", bg: "bg-amber-50" },
];
      ...
      <div>
        ...
        <div className="grid grid-cols-4 gap-4">
          {KPI_CARDS.map((card) => {
            ...
            return (
              <Card key={card.label} className="rounded-md shadow-none border-slate-200">
                <CardContent className="p-5">
                  <div className="flex items-start justify-between mb-4">
                    <div className={cn("size-9 rounded-md flex items-center justify-center", card.bg)}>
                      <Icon size={16} className={card.color} />
                    </div>
                    <div className={cn("flex items-center gap-0.5 text-xs font-bold", card.up ? "text-emerald-600" : "text-red-600")}>
                      <TrendIcon size={13} />
                      {card.trend}
                    </div>
                  </div>
                  <div className="text-2xl font-bold text-slate-900 leading-none">{card.value}</div>
                  <div className="text-xs font-medium text-slate-600 mt-1">{card.label}</div>
