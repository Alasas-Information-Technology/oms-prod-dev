import { getApprovalTasksForUser } from "@/src/lib/demo-data";
import { getActivePersonaId } from "@/src/lib/demo-data/persona-auth";
...
export function getNeedsAttentionSummary(userId?: string) {
  const effectiveUserId = userId || getActivePersonaId();
  const reqTasks = getApprovalTasksForUser(effectiveUserId);
  const reqCount = reqTasks.length;
