...
export const dashboardService = {
  ...
  async getDashboardStats(currentUser) {
    ...
    const [reqs, candidates, emiratis] = await Promise.all([
      rQuery,
      cQuery.select('id', { count: 'exact', head: true }),
  ...
  async getSidebarCounts(currentUser) {
    ...
    let oQuery = supabase.from('candidates').select('id', { count: 'exact', head: true }).eq('status', 'SELECTED');
    if (!globalView && currentUser.department_id) {
        // Similar to candidates count, we count those selected for the user's department
        // For simplicity, we filter via the same 'requisitions' join logic later or keep it global if needed
    }
